...
for x in range(0,200):
    ...
    line = s.translate(str.maketrans(intab,outtab,"0"))# deletes various strings from input
    ...
    code = f"=GOOGLEFINANCE(\"{line8}\", \"close\", DATE(2020,12,31), DATE(2021,3,31), \"DAILY\")" # this is the code I intend to import
    wks.update_value((1,i), code) # updates values using pygsheets
    j = j + 1
    i = i +2
    t.sleep(2.4)# google sheets has a limit on the amount of imports you can have per minute